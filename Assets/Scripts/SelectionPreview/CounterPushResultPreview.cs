...
{
    public class CounterPushResultPreview : MonoBehaviour {

        private float _initialY;
        private float _initialWidth;
        private float _initialHeight;

        public static CounterPushResultPreview Instance;
        ...
        {
            float offsetX = (float)attackResults[AttackResult.Kill] * _initialWidth + (float)attackResults[AttackResult.Push] * _initialWidth;

            GetComponent<RectTransform>().sizeDelta = new Vector2(_initialWidth * 4 * (float)attackResults[AttackResult.CounterPush], _initialHeight);
